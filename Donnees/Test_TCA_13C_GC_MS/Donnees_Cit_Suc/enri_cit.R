...
str(donnees)
donnees$difference_enri..cit...suc..1

figure_suc <- ggplot (donnees , aes (x = condition , y = difference_enri..cit...suc. , fill = replicat))+
...
figure_suc
figure_suc_glu <- ggplot (donnees , aes (x = condition , y = difference_enri..cit...suc..1 , fill = replicat))+
  geom_bar(stat = "identity" ,  position=position_dodge() ) +
  labs (x = "Condition",
